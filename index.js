// Rev607 result Worker full replacement for Cloudflare Workers
// Endpoint: /api/result
...
export default {
  async fetch(request) {
    ...
    try {
      ...
      return json({ ok:false, source:'', result:{wide:[]}, diagnosis:{rev606:true, rev607:true, error:String(e && e.message || e)} }, 200);
    }
  }
};
...
function parseResultHtml(html){
  ...
  const sectionHits = {
    ...
    order:/着順|確定/.test(text)
  };
  // Very conservative fallback: split by labels and only use nearby numbers.
  result.umaren = parseLabeledBet(text, /馬\s*連|馬連/, 2);
  const wideArea = sliceBetween(text, /ワイド/, /馬\s*単|3\s*連\s*複|三連複|3連複|3\s*連\s*単|三連単|3連単/);
  if (wideArea) {
    const pairs = [...wideArea.matchAll(/((?:\b(?:[1-9]|1[0-8])\b\s*){2}).{0,30}?(\d{2,9})\s*円?/g)].slice(0,3);
    result.wide = pairs.map(m => ({ combo: combo(m[1],2), amount: Number(m[2]) })).filter(x=>x.combo&&x.amount);
  }
  result.sanrenpuku = parseLabeledBet(text, /3\s*連\s*複|三連複|3連複/, 3);
  // finishing order: first 3 horse numbers in order table area if available.
  const orderArea = sliceBetween(text, /着順|確定/, /払戻|配当|単勝/);
  ...
  return { ok, result, reason: ok ? 'parsed_by_rev606_fallback' : 'html_fetched_but_result_labels_not_parsed', sectionHits };
}
